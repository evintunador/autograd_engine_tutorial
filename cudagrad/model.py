"""
A minimal GPT built entirely out of cudagrad's custom CUDA-kernel autograd engine.

This mirrors ``tritongrad/model.py`` (and the architecture in ``minigrad/model.py``):
token + position embeddings, a stack of pre-norm residual layers each with multi-head
self-attention + an MLP, a final norm and an output projection. Every bit of
forward/backward math runs through CudaTensor ops and the custom kernels in
``kernels/`` — the only difference from tritongrad is the kernel backend (raw CUDA
C++ vs Triton).
...
NOTE: like the rest of cudagrad this requires a CUDA GPU + nvcc to import/run.
"""
import math
...
def _as_int_tokens(tokens):
    """coerce a numpy/torch/CudaTensor of token ids into a CudaTensor whose .data
    lives on the engine device (the embedding kernel re-casts to int on load)."""
    if isinstance(tokens, CudaTensor):
        return tokens
    if isinstance(tokens, np.ndarray):
        tokens = torch.tensor(tokens)
    if tokens.ndim == 1:  # a single unbatched sequence -> add a batch dim
        tokens = tokens.unsqueeze(0)
    return CudaTensor(tokens.to(DEVICE))
...
class GPT(nn.Module):
    def __init__(self, config, device=None):
        ...
        # constant (non-trainable) scalars, kept as (1,)-shaped CudaTensors so they
        # flow through our broadcasting binary-op kernels without hitting torch ops
        self.scale = CudaTensor(torch.tensor([config['model_dim'] ** -0.5], device=self.device),
                                requires_grad=False)
        ...
        self.eps = CudaTensor(torch.tensor([1e-9], device=self.device), requires_grad=False)
    ...
    def cross_entropy(self, probabilities, targets):
        ...
        Built purely from differentiable CudaTensor ops (we can't gather via
        ...
        one_hot = CudaTensor(one_hot_data, requires_grad=False)
...
if __name__ == "__main__":
    # tiny smoke test (requires a GPU).
    config = {
        'vocab_len': 65,
        'model_dim': 64,
        'max_seq_len': 16,
