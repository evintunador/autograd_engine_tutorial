"""
Character-level training of cudagrad's CUDA-kernel GPT on tiny-shakespeare
(../input.txt).

This mirrors ``tritongrad/train.py`` but runs on raw-CUDA-kernel CudaTensors. The
goal is simply to demonstrate that the engine learns: the training loss should fall
well below its random-init starting point of ln(vocab_len).

Unlike tritongrad, there is NO autotune warmup dance — cudagrad's CUDA kernels don't
autotune, so each backward accumulates exactly once into zeroed grads and a plain
training loop is correct. (The very first iteration is still slow: nvcc JIT-compiles
the extension on first use.)

REQUIRES a CUDA GPU + nvcc. Run from the ``cudagrad/`` directory:
    python train.py
"""
import os
import time
...
def greedy_inference(model, prompt, gen_len):
    ...
    with torch on the raw probability data rather than building a dedicated kernel —
    ...
    for _ in range(gen_len):
        ...
        probs, _ = model(inp)
        next_id = int(torch.argmax(probs.data[0, -1]).item())
        toks.append(next_id)
...
if __name__ == "__main__":
    # ---- training loop ---------------------------------------------------
    # The first iteration is slow because nvcc JIT-compiles the CUDA extension on
    # first use; every iteration after that is fast.
    print("first iteration triggers the nvcc compile (slow); then training is fast...")
    start = time.time()
