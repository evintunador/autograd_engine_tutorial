...
def matmul_fwd_m_by_m(
    ...
    stride_am, stride_ak, # tuple of how much to increase the ptr by when moving by 1 element along that dimension
    stride_bk, stride_bn, 
    stride_cm, stride_cn,
    # meta-parameters
    ...
    offsets_am = (pid_m * BLOCK_SIZE_M + tl.arange(0, BLOCK_SIZE_M)) % M #<- not sure why we modulo here, is it related to the mask?
    # b_ptrs is a block of [BLOCK_SIZE_K, BLOCK_SIZE_N] pointers
    offsets_bn = (pid_n * BLOCK_SIZE_N + tl.arange(0, BLOCK_SIZE_N)) % N #<- not sure why we modulo here, is it related to the mask?
    ...
    for k in range(0, tl.cdiv(K, BLOCK_SIZE_K)): # iterate from 0 to the number of blocks along K dimension
        ...
        mask_k = offsets_k < K - k * BLOCK_SIZE_K
            ...
            # when that does happen, suddenly now K - k * BLOCK_SIZE_K gives us a number of entries <= BLOCK_SIZE_K
        ...
        # the masks along m and n dimensions are simpler
        #mask_a = (offsets_am.expand_dims(1) < M) + mask_k.expand_dims(0)
        #mask_b = mask_k.expand_dims(1) + (offsets_bn.expand_dims(0) < N)
        ...
        a = tl.load(a_ptrs, mask=mask_k.expand_dims(0), other=0.0)
        b = tl.load(b_ptrs, mask=mask_k.expand_dims(1), other=0.0)
