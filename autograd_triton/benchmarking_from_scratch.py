...
def benchmark_hadamard(
    ...
    for case in benchmark_cases:
            ...
            for size in case["sizes"]:
                ...
                if provider == 'triton':
                    ...
                    if mode == "bwd":
                        ...
                        dA = torch.randn_like(inputs[0])
                        dB = torch.randn_like(inputs[1])

                        fn = lambda: hadamard.binary_op_backward[grid](
                            inputs[0], inputs[1],
                            dA, dB, dO, 
                            n_elements, loop_stride,
                            OP=op_name,
        ...
        csv_path = f'./benchmarks_from_scratch/{plot_name}.csv'
        ...
        plt.legend()
        plt.grid(True)
        plt.savefig(f'./benchmarks_from_scratch/{plot_name}.png')
        plt.close()
...
if __name__ == "__main__":
    ...
    if args.all or args.sub:
        ...
        def sub_shapes(size, broadcasting): 
            return [(size, size), (size,)] if broadcasting else [(size, size), (size, size)] 
        def flops_calc_sub(shapes: list, ms: float):
            # TODO separate bw fwd & bwd
            flops = sum(2 * prod(shape) for shape in shapes)
            return flops * 1e-12 / (ms * 1e-3)
        benchmark_hadamard(
            torch_sub, triton_sub,
            op_name="sub", 
            input_shapes_fn=sub_shapes,
            flops_calc_fn=flops_calc_sub
        )
    ...
    if args.all or args.mul:
        ...
        def mul_shapes(size, broadcasting): 
            return [(size, size), (size,)] if broadcasting else [(size, size), (size, size)] 
        def flops_calc_mul(shapes: list, ms: float):
            # TODO separate bw fwd & bwd
            flops = sum(2 * prod(shape) for shape in shapes)
            return flops * 1e-12 / (ms * 1e-3)
        benchmark_hadamard(
            torch_mul, triton_mul,
            op_name="mul", 
            input_shapes_fn=mul_shapes,
            flops_calc_fn=flops_calc_mul
        )
    ...
    if args.all or args.div:
        ...
        def div_shapes(size, broadcasting): 
            return [(size, size), (size,)] if broadcasting else [(size, size), (size, size)] 
        def flops_calc_div(shapes: list, ms: float):
            # TODO separate bw fwd & bwd
            flops = sum(2 * prod(shape) for shape in shapes)
            return flops * 1e-12 / (ms * 1e-3)
        benchmark_hadamard(
            torch_div, triton_div,
            op_name="div", 
            input_shapes_fn=div_shapes,
            flops_calc_fn=flops_calc_div
        )
